...
document
  .getElementById("select-file-input")
...
var roomid = loc.search;
roomid = roomid.split("=")[1];
console.log(roomid);
document.getElementById("mid").innerText = roomid;
document.getElementById("link").innerText = loc;
var endPoint = wsStart + loc.host + loc.pathname + roomid;
  ...
  .then((e) => {
    btnShareScreen.onclick = (event) => {
      if (screenShared) {
        ...
        localVideo.srcObject = localStream;
        btnShareScreen.innerHTML = '<i class="fas fa-share-square"></i>';
  ...
  .then((e) => {
    btnRecordScreen.addEventListener("click", () => {
      if (recording) {
        ...
        btnRecordScreen.innerHTML = '<i class="fas fa-record-vinyl"></i>';
...
function createVideo(peerUsername) {
  ...
  // videoContainer.style.position = 'position: relative;vertical-align: middle; align-self: center; border-radius: 10px;overflow: hidden; display: inline-block; margin: 1px; animation: show 0.4s ease;';
  ...
  lable.style = "position:fixed; color: wheat; font-size: 45px;";
  lable.innerText = peerUsername;
  ...
  videoWrapper.appendChild(lable);
